...
def link(zone: Zone, bars: list[Bar], registry: list[Pool], price: float,
         window: int = 2) -> tuple[Pool | None, str]:
    """1A: highest-LPS registry pool swept in the `window` bars immediately before the displacement (GEM1: 2)."""
    if zone.disp_idx is None:
        return None, "UNLINKED"
    side = "low" if zone.direction == "buy" else "high"
    tol = lv.TOUCH_TOL_PCT * price
    best: tuple | None = None
    for k in range(zone.disp_idx - 1, zone.disp_idx - 1 - window, -1):
        if k < 0:
            continue
        b = bars[k]
        for p in registry:
...
def candidates(pdas: dict[str, dict[str, list[Zone]]], bars: dict[str, list[Bar]], registry: list[Pool],
               bias: str, price: float, lth: str, ith: str, window: int = 2,
               keep_touched: bool = False) -> tuple[list[Candidate], list[Candidate]]:
    """Linked and UNLINKED candidates in the bias direction that pass Filter 1 (displacement). `keep_touched`
    (intraday mode, D-70): a touched zone stays a candidate — unmitigated scores 0 (1B) — until a bar closes
    beyond its far edge."""
    linked, unlinked = [], []
    ...
    for tf in ("D1", "H4", "H1"):
        ...
        for kind in CHAIN_KINDS:
            for z in pdas[tf][kind]:
                ...
                if kind != "IFVG" and not unmit and not (keep_touched and not broken(z, tfb)):
                    continue
                d = z.disp_idx
                ...
                c.gen, c.sweep_type = link(z, tfb, registry, price, window)
                c.breakdown = {
...
def assign_chains(cands: list[Candidate], gates: tuple[int, int, int]) -> dict[str, Candidate]:
    ...
    if b:
        ...
        used.add(id(b))
    return _add_c(chains, ordered, used, gates[2])


def _add_c(chains: dict[str, Candidate], ordered: list[Candidate], used: set[int], gate: int) -> dict:
    if len(chains) < 2:
        return chains  # C is a *third* zone
    prior = list(chains.values())
    for c in ordered:
        if id(c) in used or c.gen_lps < gate:
            continue
...
INTRADAY_MIN_LPS = 20  # D-70: floor for a zone generated by an intraday (H1) sweep


def broken(z: Zone, bars: list[Bar]) -> bool:
    """A bar closed beyond the zone's far edge after it formed (the PDA failed)."""
    start = bisect.bisect_right(bars, z.formed_at, key=lambda b: b.t)
    far_buy, far_sell = z.low, z.high
    return any((b.c < far_buy) if z.direction == "buy" else (b.c > far_sell) for b in bars[start:])


def distance(c: Candidate, price: float, bias: str) -> float | None:
    """How far price must retrace to reach the zone; None when the zone lies on the wrong side of price."""
    z = c.zone
    if bias == "sell":
        return None if z.high < price else max(0.0, z.low - price)
    return None if z.low > price else max(0.0, price - z.high)


def assign_intraday(cands: list[Candidate], gates: tuple[int, int, int], price: float, reach: float,
                    bias: str) -> dict[str, Candidate]:
    """D-70 (owner's intraday mode): zones price can reach this session come first, ranked by the 1C priority
    (generating LPS, displacement, TF, type, unmitigated); the GEM1 gates only fill the slots left over."""
    ordered = sorted(cands, key=Candidate.order_key)
    near = [c for c in ordered if c.gen_lps >= INTRADAY_MIN_LPS
            and (d := distance(c, price, bias)) is not None and d <= reach]
    near_ids = {id(c) for c in near}
    far = [c for c in ordered if c.gen_lps >= INTRADAY_MIN_LPS and id(c) not in near_ids]
    chains: dict[str, Candidate] = {}
    used: set[int] = set()
    # near zones first; empty slots take the best remaining zones, so CHAIN_A (the GEM2 thesis root) always exists
    for name, c in zip(("CHAIN_A", "CHAIN_B"), near + far, strict=False):
        chains[name], c.chain = c, name[-1]
        used.add(id(c))
    return _add_c(chains, ordered, used, gates[2])
